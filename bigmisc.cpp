...
static int row, col, exprow, expcol;
static enum { ININT, INDEC, INEX } mode;
...
int Normalize (int w)
{
   ...
   *(wr + 1) = 1;               /*  simplify next statement */
   ...
   ldigits = (wr - wl) + 1;      /* Number of significant digits */
   work[w].digits = ldigits;

   shift = wl - &work[w].man[0]; /* Normalize mantissa if needed */
   if (shift) {
      memmove (work[w].man, wl, (ldigits * sizeof (WORKDIGIT)));
      work[w].exp -= (long) shift;
   }
   /* Zero digits beyond mantissa */

   memset (&work[w].man[ldigits], 0,
      ((workprec - ldigits) * sizeof (WORKDIGIT)));
...
int ExtendedIntegerPart (void)
{
   ...
   ldigits = work[0].digits;

   if ((!ldigits)                /* If it's zero,       */
      ||(work[0].exp >= (long) ldigits)) { /*  or already integer */
   ...
   /* Zero decimal part */
   ldigits = (int) work[0].exp;
   memset (&work[0].man[ldigits], 0,
      ((workprec - ldigits) * sizeof (WORKDIGIT)));
   work[0].digits = ldigits;
...
int ExtendedFractionPart (void)
{
   ...
   if (work[0].exp >= (long) ldigits) { /* All integer,  */
      ClearWork (0);            /*  return zero  */
      return (TRUE);
   }
   /* Zero integer part */
   ldigits = (int) work[0].exp;
   memset (&work[0].man[0], 0, (ldigits * sizeof (WORKDIGIT)));
...
//                                                
//  returns:
//    < 0 means error or user abort
//    = 0 means still entering X value
//    > 0 means done
//************************************************

//lint -esym(578, chr)  Declaration of symbol 'chr' hides symbol 'chr' (line 118, file bigcalc.cpp)
...
void MoveStackWork (int source, int dest)
{
   ...
   if ((size = stack[source].digits)) {
      ...
      if (size < workprec)
         memset (&work[dest].man[size], 0,
            ((workprec - size) * sizeof (WORKDIGIT)));
   }
   else
      ClearWork (dest);
...
void MoveWorkStack (int source, int dest)
{
   ...
   if ((size = work[source].digits)) {
      ...
      if (size < normprec)
         memset (&stack[dest].man[size], 0,
            ((normprec - size) * sizeof (NORMDIGIT)));
...
void MoveWorkWork (int source, int dest)
{
   ...
   if ((size = work[source].digits)) {
      ...
      if (size < compprec)
         memset (&work[dest].man[size], 0,
            ((compprec - size) * sizeof (WORKDIGIT)));
   }
   else
      ClearWork (dest);
